...
<div class="px-6 sm:px-8 py-8 max-w-7xl mx-auto space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-stone-200 pb-5">
        <div>
            ...
            <h1 class="serif-title text-3xl font-bold text-stone-950 mt-1">Términos y Condiciones por Secciones</h1>
            <p class="text-stone-600 text-sm mt-1">Edita el contrato dividiéndolo en secciones con títulos claros. Los cambios se actualizarán automáticamente en la tienda.</p>
        </div>
        <div class="flex items-center space-x-3">
            <a href="{{ route('terminos') }}" target="_blank" class="inline-flex items-center space-x-2 px-4 py-2.5 bg-amber-800 hover:bg-amber-900 text-white text-xs font-bold rounded-xl transition-all shadow-md">
    ...
    <!-- Formulario Editor de Términos por Secciones -->
    <form action="{{ route('admin.terminos.actualizar') }}" method="POST" id="form-terminos" class="space-y-6">
        @csrf
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   ...
                                   placeholder="Ej. 1. Aceptación de Términos Legales" 
                                   required 
                        ...
                        <div>
                            ...
                            <span class="text-[11px] text-stone-400 block mt-1">💡 Usa saltos de línea para separar párrafos. Las líneas con "•" o "-" se mostrarán como viñetas con viñetas destacadas en el sitio web.</span>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        <div class="text-center pt-2">
            <button type="button" onclick="agregarNuevaSeccion()" class="inline-flex items-center space-x-2 px-6 py-3.5 bg-stone-100 hover:bg-stone-200 border-2 border-dashed border-stone-300 text-stone-800 font-bold text-xs uppercase tracking-wider rounded-2xl transition-all shadow-xs">
                ...
                <span>+ Agregar Otra Sección al Contrato</span>
            </button>
        </div>
...
<script>
        ...
        card.innerHTML = `
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" 
                           name="secciones[${index}][titulo]" 
                           value="${numDisplay}. Nueva Sección Legal" 
    ...
    function eliminarSeccion(btn) {
        ...
        if (total <= 1) {
            alert('Debes mantener al menos una sección en el contrato.');
            return;
        }
        if (confirm('¿Estás seguro de que deseas eliminar esta sección del contrato?')) {
            card.remove();
            actualizarNumerosSecciones();
        }
    }
