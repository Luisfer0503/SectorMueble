...
@section('contenido')
    ...
    <div class="pb-6 border-b border-zinc-200 mb-8">
        <nav class="flex text-xs font-medium text-zinc-500 space-x-2 mb-2">
            ...
            <span>/</span>
            <span class="text-zinc-800">Crear</span>
        </nav>
        <h1 class="serif-title text-3xl font-bold text-zinc-950">Crear Cupón de Descuento</h1>
        <p class="text-zinc-500 text-sm mt-1">Crea un nuevo código de descuento aplicable al checkout.</p>
    </div>

    <!-- Form -->
    <div class="bg-white border border-zinc-200 rounded p-6 sm:p-8 shadow-sm max-w-lg">
        <form action="{{ route('admin.cupones.guardar') }}" method="POST" class="space-y-6">
            @csrf

            <!-- Codigo -->
            <div>
                <label for="codigo" class="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1">Código del Cupón</label>
                <input type="text" name="codigo" id="codigo" required value="{{ old('codigo') }}" placeholder="Ej: DESCUENTO15" class="w-full bg-zinc-50 border border-zinc-200 rounded text-sm px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-amber-700 font-mono tracking-wider">
                @error('codigo')
            ...
            <div>
                ...
                <select name="tipo" id="tipo" required class="w-full bg-zinc-50 border border-zinc-200 rounded text-sm px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-amber-700">
                    ...
                    <option value="fijo" {{ old('tipo') === 'fijo' ? 'selected' : '' }}>Descuento Fijo ($ MXN)</option>
                </select>
                @error('tipo')
                    <span class="text-xs text-rose-600 font-medium mt-1 block">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="number" step="0.01" name="valor" id="valor" required value="{{ old('valor') }}" placeholder="Ej: 10 o 500" class="w-full bg-zinc-50 border border-zinc-200 rounded text-sm px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-amber-700">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="activo" id="activo" checked value="1" class="h-4 w-4 text-amber-850 border-zinc-300 rounded focus:ring-amber-700">
                <label for="activo" class="ml-2 text-xs text-zinc-700 font-medium cursor-pointer">Activar este cupón inmediatamente</label>
            </div>
