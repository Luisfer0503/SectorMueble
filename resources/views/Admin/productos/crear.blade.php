...
@section('contenido')
    ...
    <div class="bg-white border border-zinc-200 rounded p-6 sm:p-8 shadow-sm max-w-3xl">
        <form action="{{ route('admin.productos.guardar') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="url" name="imagen_url" id="imagen_url" required value="{{ old('imagen_url') }}" placeholder="https://images.unsplash.com/..." class="w-full bg-zinc-50 border border-zinc-200 rounded text-sm px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-amber-700">
                    @error('imagen_url')
                        <span class="text-xs text-rose-600 font-medium mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
